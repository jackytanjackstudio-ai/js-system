...
export async function POST(req: Request) {
  ...
  const record = await prisma.seasonalStrategy.create({
    data: {
      ...
      moodboard:         JSON.stringify(body.moodboard ?? []),
      videoLinks:        JSON.stringify(body.videoLinks ?? []),
...
export async function PATCH(req: Request) {
  ...
  const record = await prisma.seasonalStrategy.update({
    ...
    data: {
      ...
      ...(body.moodboard   !== undefined && { moodboard:   JSON.stringify(body.moodboard) }),
      ...(body.videoLinks  !== undefined && { videoLinks:  JSON.stringify(body.videoLinks) }),
