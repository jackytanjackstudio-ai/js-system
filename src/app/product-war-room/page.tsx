...
import {
  ...
  TrendingUp, AlertTriangle, Lightbulb, Package,
} from "lucide-react";
...
// ─── Tab: Sourcing Pool ─────────────────────────────────────────────────────
function SourcingPool({ products, onAdvance, onDelete, canEdit }: {
  products: Product[];
      ...
      {products.map(p => {
        ...
        return (
          <div key={p.id} className="card space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2 text-center bg-gray-50 rounded-xl p-2">
                {p.targetPrice && (
                  <div>
                    ...
                    <div className="text-xs font-bold text-gray-800">RM{p.targetPrice}</div>
                  </div>
                )}
                {p.cost && (
                  <div>
                    <div className="text-[10px] text-gray-400">Est Cost</div>
                    <div className="text-xs font-bold text-gray-800">RM{p.cost}</div>
                  </div>
                )}
                {m !== null && (
                  <div>
                    <div className="text-[10px] text-gray-400">Margin</div>
                    <div className={`text-xs font-bold ${m >= 50 ? "text-green-600" : m >= 30 ? "text-amber-600" : "text-red-500"}`}>{m}%</div>
                  </div>
                )}
              </div>
            )}
            ...
            {p.signalSource && (
              <div className="text-[10px] text-gray-400">Source: {p.signalSource}</div>
            )}
            {p.notes && (
            ...
            {canEdit && (
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => onAdvance(p.id, "Validating")}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-brand-500 hover:bg-brand-600 text-white rounded-xl text-xs font-semibold transition-colors">
...
function ValidationTab({ products, outlets, onSubmitValidation, onAdvance, canEdit }: {
  ...
  function getForm(productId: string) {
    return forms[productId] ?? { outletId: "", confidence: 0, wouldSell: true, expected: 10, reason: "", submitting: false };
  }
  function setForm(productId: string, patch: Partial<ReturnType<typeof getForm>>) {
    setForms(prev => ({ ...prev, [productId]: { ...getForm(productId), ...patch } }));
  }
          ...
          <div key={p.id} className="card space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  <span className="badge bg-gray-100 text-gray-600">{p.category}</span>
                  <span className="badge bg-amber-50 text-amber-600">Demand: {p.demandScore}/100</span>
                  {avgConf && <span className="badge bg-green-50 text-green-600">Avg Confidence: {avgConf}/5</span>}
                </div>
              </div>
              {canEdit && (
                <button
                  onClick={() => onAdvance(p.id, "Reserving")}
                  className="ml-3 text-xs font-semibold px-3 py-1.5 bg-brand-100 hover:bg-brand-200 text-brand-700 rounded-lg transition-colors flex-shrink-0">
                  → Reserve
                </button>
              )}
            </div>
            ...
            <div className="border-t border-gray-100 pt-3">
              <button
                ...
                {p.validations.length > 0 ? "Add / update your store's validation" : "Submit validation for a store"}
              </button>
                ...
                <div className="mt-3 space-y-3 bg-gray-50 rounded-xl p-4">
                  ...
                  <div>
                    ...
                    <textarea className="textarea text-sm" rows={2}
                      placeholder='"Customer always asks for this size…"'
                      value={form.reason}
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      await onSubmitValidation(p.id, {
                        ...
                        outletName: outlet?.name ?? form.outletId,
                        confidenceScore: form.confidence,
                        wouldSell: form.wouldSell,
                        expectedSales: form.expected,
...
function ReservationTab({ products, outlets, onSubmitReservation, onAdvance, canEdit }: {
  products: Product[];
  outlets: Outlet[];
  onSubmitReservation: (productId: string, data: object) => Promise<void>;
  onAdvance: (id: string, status: string) => Promise<void>;
  canEdit: boolean;
}) {
  ...
  function getQty(productId: string, outletId: string, fallback: number) {
    return qtys[productId]?.[outletId] ?? String(fallback);
  }
  function setQty(productId: string, outletId: string, val: string) {
    setQtys(prev => ({ ...prev, [productId]: { ...(prev[productId] ?? {}), [outletId]: val } }));
  }
        ...
        return (
          <div key={p.id} className="card space-y-4">
              ...
              {activeOutlets.map(outlet => {
                ...
                return (
                  <div key={outlet.id} className="flex items-center gap-3">
                    ...
                    />
                    <button
                      disabled={saving[`${p.id}-${outlet.id}`]}
                      onClick={async () => {
                        setSaving(prev => ({ ...prev, [`${p.id}-${outlet.id}`]: true }));
                        await onSubmitReservation(p.id, {
                          outletId: outlet.id,
                          outletName: outlet.name,
                          quantity: parseInt(val) || 0,
                        });
                        setSaving(prev => ({ ...prev, [`${p.id}-${outlet.id}`]: false }));
                      }}
                      ...
                      {saving[`${p.id}-${outlet.id}`] ? <Loader2 size={10} className="animate-spin" /> : "Save"}
                    </button>
            ...
            <div className="bg-gray-50 rounded-xl p-3 flex items-center justify-between">
              <div>
                <span className="text-xs text-gray-500">Stores participated: </span>
                <span className="text-xs font-bold text-gray-800">
                  {p.reservations.filter(r => r.quantity > 0).length} / {activeOutlets.length}
                </span>
              </div>
              <div className="text-sm font-black text-brand-600">{totalRes} units total</div>
            </div>

            {canEdit && (
              <button
                onClick={() => onAdvance(p.id, "Watchlist")}
  ...
  return (
    <div className="space-y-4">
      {products.map(p => {
        ...
        const isConservative = dec.go === "go_conservative";

        return (
          <div key={p.id} className={`card border-2 space-y-4 ${
            isGo ? "border-green-200" : isConservative ? "border-amber-200" : "border-gray-100"
          }`}>
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="text-right flex-shrink-0">
                <div className={`text-3xl font-black ${isGo ? "text-green-600" : isConservative ? "text-amber-600" : "text-gray-400"}`}>
                  {dec.final}
                </div>
            ...
            <div className={`rounded-xl p-4 ${
              isGo ? "bg-green-50 border border-green-200" : isConservative ? "bg-amber-50 border border-amber-200" : "bg-gray-50 border border-gray-200"
            }`}>
              <div className={`text-xs font-bold uppercase tracking-wider mb-2 ${
                isGo ? "text-green-700" : isConservative ? "text-amber-700" : "text-gray-500"
            ...
            {/* Stats */}
            {(p.validations.length > 0 || p.reservations.length > 0) && (
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <div className="bg-gray-50 rounded-xl p-2">
                  ...
                  <div className="text-[10px] text-gray-400">Units Reserved</div>
                </div>
                <div className="bg-gray-50 rounded-xl p-2">
...
function TrackingTab({ products, onStatusChange, canEdit }: {
  ...
  const statusConfig: Record<string, { color: string; bg: string; label: string }> = {
    Testing:    { color: "text-blue-700",  bg: "bg-blue-100",  label: "Testing"    },
    Scale:      { color: "text-green-700", bg: "bg-green-100", label: "Scale"      },
    Eliminated: { color: "text-red-600",   bg: "bg-red-100",   label: "Eliminated" },
  };
        ...
        return (
          <div key={p.id} className="card">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {/* Status */}
              <span className={`status-pill flex-shrink-0 ${sc.bg} ${sc.color}`}>{sc.label}</span>

              {/* Stage upgrade for Testing → Scale */}
              {canEdit && p.status === "Testing" && (
                <div className="flex gap-1.5 flex-shrink-0">
                  <button
                    onClick={() => onStatusChange(p.id, "Testing", "Cannonball")}
                    className="text-[10px] font-semibold px-2 py-1 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors">
                    → Cannonball
                  </button>
                  <button
                    onClick={() => onStatusChange(p.id, "Scale")}
                    className="text-[10px] font-semibold px-2 py-1 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors">
                    → Scale
                  </button>
                  <button
                    onClick={() => onStatusChange(p.id, "Eliminated")}
                    className="text-[10px] font-semibold px-2 py-1 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors">
                    Eliminate
                  </button>
                </div>
              )}
              {canEdit && p.status === "Scale" && (
                <button
                  onClick={() => onStatusChange(p.id, "Eliminated")}
                  className="text-[10px] font-semibold px-2 py-1 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors flex-shrink-0">
                  Discontinue
                </button>
              )}
            </div>

            {/* Cannonball ready indicator */}
...
function AddProductModal({ onClose, onSave }: {
  ...
  const [form, setForm] = useState({
    ...
    useCase: [] as string[], style: "", signalSource: "", notes: "",
    demandScore: "50",
  });
  ...
  async function submit() {
    ...
    await onSave({
      ...
      signalSource: form.signalSource || null,
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 space-y-5"
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input className="input" placeholder="e.g. Functional, Formal, Casual" value={form.style}
              onChange={e => setForm(f => ({ ...f, style: e.target.value }))} />
          </div>

          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Signal Source</label>
            <input className="input" placeholder="e.g. Customer demand, TikTok trend" value={form.signalSource}
...
export default function ProductWarRoom() {
  ...
  const canEdit = ["admin", "product"].includes(user?.role ?? "");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-1.5 text-[10px] text-gray-400 overflow-x-auto">
        {["Sourcing", "→", "Validation", "→", "Reservation", "→", "Decision", "→", "Bullet", "→", "Cannonball / Scale"].map((s, i) => (
      ...
      {loading ? (
        ...
        <>
          {activeTab === "sourcing" && (
            <SourcingPool products={sourcing} onAdvance={advanceStatus} onDelete={deleteProduct} canEdit={canEdit} />
          )}
